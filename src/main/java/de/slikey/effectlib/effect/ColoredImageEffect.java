...
public class ColoredImageEffect extends Effect {
    ...
    public void onRun() {
        ...
        for (int y = 0; y < image.getHeight(); y += stepY) {
            for (int x = 0; x < image.getWidth(); x += stepX) {
                ...
                ParticleEffect.REDSTONE.display(getColor(r, g, b), location.add(v), visibleRange);
                location.subtract(v);
            }
        }
        rotationStep++;
    ...
    public ParticleEffect.ParticleColor getColor(int r, int g, int b) {
        return new ParticleEffect.OrdinaryColor(r, g, b);
    }
}